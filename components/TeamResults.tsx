...
const TeamCard: React.FC<{ team: Team, sortByRating: boolean }> = ({ team, sortByRating }) => (
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 flex-grow overflow-y-auto pr-2">
                {teams.map(team => (
                    <TeamCard key={team.id} team={team} sortByRating={sortByRating} />
                ))}
            </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {unassignedPlayers.map(p => (
                            <span key={p.id} className="bg-slate-700 text-slate-300 px-3 py-1 rounded-full text-sm">
                                ...
                                {sortByRating && (
                                    <>
                                        {' '}({p.rating} <StarIcon className="inline-block w-3 h-3 text-yellow-400 -mt-1" />)
                                    </>
                                )}
                            </span>
                        ))}
